...
public class MakeGoBackAction : DT_Action
{
    public override Queue<Vector3> MakeAction(PlayerAI player)
    {
        ...
        escapePosition = new Vector3(Mathf.Clamp(escapePosition.x,0 + player.MapData.OriginPoint.x,19.5f + player.MapData.OriginPoint.x),0, Mathf.Clamp(escapePosition.z, 0 + player.MapData.OriginPoint.z, 19.5f + player.MapData.OriginPoint.z));
